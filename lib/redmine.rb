...
Redmine::AccessControl.map do |map|
  ...
  map.permission :set_issue_relations, {:issue_relations => [:new, :destroy]}, :require => :loggedin
  ...
  # Queries
  map.permission :manage_queries, {:queries => [:new, :edit, :destroy]}, :require => :member
  ...
  map.permission :view_spent_time, :timelog => [:details, :report]
  # News
  ...
  # Wiki
  map.permission :view_wiki, :wiki => [:index, :history, :diff, :special]
  map.permission :edit_wiki, :wiki => [:edit, :preview, :add_attachment, :destroy_attachment]
  map.permission :delete_wiki_pages, {:wiki => :destroy}, :require => :member
  # Message boards
  map.permission :view_boards, {:boards => [:index, :show], :messages => [:show]}, :public => true
  map.permission :post_messages, {:messages => [:new, :reply]}, :require => :loggedin
...
Redmine::MenuManager.map :project_menu do |menu|
  ...
  menu.push :label_board_plural, { :controller => 'boards', :action => 'index' }, :param => :project_id, :if => Proc.new { |p| !p.boards.empty? }
